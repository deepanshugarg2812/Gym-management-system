...
public class Login extends HttpServlet{
	...
	public Owner isValid(String username,String password,Connection connnection) {
		Owner owner = null;
		try {
			PreparedStatement statement = connnection.prepareStatement("select * from Owner where username = ? and password = ?");
	...
	public void doPost(HttpServletRequest req,HttpServletResponse resp) throws ServletException, IOException {
		...
		if(username.isPresent()==false || password.isPresent()==false) {
			...
			rd = req.getRequestDispatcher("ownerlogin");
			rd.forward(req, resp);
		...
		Owner exist = isValid(username.get(), password.get(), connection);
		
		if(exist!=null) {
			//send forward to the owner home
			HttpSession session = req.getSession();
			session.setAttribute("owner", exist);
			resp.sendRedirect("ownerhome");
		}
